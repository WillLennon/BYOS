...
# Check if this is before or after reboot
$username = Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name DefaultUserName
$password = Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name DefaultPassword

if ([string]::IsNullOrEmpty($username) -or [string]::IsNullOrEmpty($password))
{
    Log-Message "Registry keys not found"

    Log-Message "Disabling Windows Update"
    ...
    if ($runAsUser)
    {
        ...
        $username = 'AzDevOps'
        $password = 'A1b2838!K#n0sn0abc' #(New-Guid).ToString()                   TODO FIX THIS!
       ...
       if ($interactive)
       {
          ...
          Set-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce" -Name AzDevOps -Value $cmd
...
{
    Log-Message "Back from Reboot"
    # Read the other registry settings
    $url = Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name Url
    $pool = Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name Pool
    $token = Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name Token
